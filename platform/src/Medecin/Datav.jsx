      ...
      const fetchPatients = async () => {
        try {
          ...
          const response = await fetch(
            `http://localhost:5000/find/rendezvous?medecin_id=${currentMedecinId}&date_rv=${today}&type_consultation=Consultation%20en%20ligne%20(Vidéo)`
          );
          const data = await response.json();
  
          if (response.ok) {
            setPatients(data.patients);
            console.log("thisis datass",data.patients[0]._id)
          } else {
