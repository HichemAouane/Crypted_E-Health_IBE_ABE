...
import SidebarListP from "./SidebarListP";
...
const ChatM = () => {
  ...
  const medicId = sessionStorage.getItem("profile_id");
  const [recipientId, setRecipientId] = useState(patientIdFromUrl || null);
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/get_pats_for_meds?medic_id=${medicId}`)
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/get_medics?medic_id=${medicId}`)
      .then(response => {setMedics(response.data);console.log("qaqaq",response.data)})
    ...
    <div>
      ...
      <main style={{marginTop:"115px"}} className={`main-content ${isSidebarOpen ? "expanded" : "collapsed"}`}>
        ...
        {recipientId ? (
            ...
            {recipientId ? (
              <>
                ...
                <div className="input-containerd">
                  ...
                  />
                  <button onClick={sendMessage} className="btncolorm">
          ...
          <SidebarList  medics={medics} patients={patients} setRecipientId={setRecipientId}  selectedRecipient={recipientId}  />
          </>
        ) : (
          <SidebarListP  medics={medics} patients={patients} setRecipientId={setRecipientId}  selectedRecipient={recipientId}  />
            )}
          </div>
