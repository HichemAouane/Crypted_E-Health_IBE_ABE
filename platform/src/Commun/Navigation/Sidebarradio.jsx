...
const Sidebarradio = ({ isOpen }) => {
  ...
  const handleCardSelection = (select) => {
    if (select === "ChatM") {
      ...
      navigate("/listpatradio");
    } else if (select === "Rdvactuel") {
      navigate("/rdvactuelrad");
    } else if (select === "Rdvrecep") {
        navigate("/rdvreceprad");
    } else if (select === "Home") {
      navigate("/PrincR");
    } else if (select === "Consultation des images") {
      navigate("/Consultationimages");
    } else if (select === "Compte-rendue") {
      navigate("/Compterendue");
    }
  };
  ...
  return (
    <aside className={`sidebarm ${isOpen ? "open" : "closed"}`}>
      <ul>
        ...
        {/* Élément parent exman_radiologique avec flèche */}
        <div onClick={toggleConsult} className="sidebar-parent-item">
          <SidebarItem
    icon={<img src="https://cdn-icons-png.flaticon.com/512/2418/2418174.png" 
      alt="Consultation Icon" 
      style={{ width: "40px", height: "40px", marginRight: "40px" }} />}
      text="Examen Radiologique"
      extraIcon={<FaChevronDown className={`arrow ${isConsultOpen ? "open" : ""}`} />}
          />
        </div>
        ...
        {isConsultOpen && (
          <ul className="submenum">
            <li onClick={() => handleCardSelection("Consultation des images")}>
              <SidebarItem
                ...
                text="Consultation des images"
              />
            </li>
            <li onClick={() => handleCardSelection("Compte-rendue")}>
              <SidebarItem
